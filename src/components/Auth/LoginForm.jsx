...
function LoginForm() {
    ...
    const onSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            const login = await axios.post(`${config.API_URL}/auth/landing/login`, values, { withCredentials: true });
            if (login) {
                ...
                {({ values, setFieldValue, errors, dirty, isValid }) => {
                    return (
                        <Form>
                            ...
                            <button className="btn btn-lg btn-primary w-100" type="submit" disabled={!(dirty && isValid)}>
                                Sign In
                            </button>
                        </Form>
                    )
