...
function Registration(props) {
    ...
    const initialValues = {
        ...
        noOfTickets: 1,
        fullName: user && user.data && (user.data.firstName + " " + user.data.lastName) || "",
    ...
    const validationSchema = Yup.object({
        ...
        pincode: Yup.string().matches(/^[0-9]{6}$/, 'Must be exactly 6 digits').required("Pincode is required"),
        team: Yup.string().when('eventType', {
            is: (type) => type == 'tournament',
            then: () => Yup.string().required("Team name is required")
        }),
        members: event && event.eventType == "tournament" ? Yup.array().of(
            Yup.object().shape({
                name: Yup.string().required("Name is required"),
            })) : null,
        noOfTickets: Yup.number().when('ticketSystem', {
            is: (check) => check == true,
            then: () => Yup.number().positive().required("No. of Tickets are required")
        }),
    ...
    const onSubmit = async (values, { setSubmitting, resetForm }) => {
        ...
        if (user && user.code == "unauthorised") {
            ...
            const _data = { ...values, amount: values.noOfTickets * values.amount * 100 }
            axios.post(`${config.API_URL}/landing/payments/orders`, _data, { withCredentials: true })
                .then(res => {
            ...
            {user && user.code == "unauthorised" ?
                ...
                <>
                    <div className="modal-header">
                        <div className="mdl-title mb-1"><h4 className="modal-header-title" style={{ fontSize: "24px" }}>{event.eventType} Registration</h4>
                            <p>
                                Please book for your {event.eventType} by filling the form below, specify the expected number joining the  {event.eventType}.</p>
                        </div>
                    </div>
                    <div className="modal-body">
                        <Formik
                            ...
                            {({ values, setFieldValue, errors, dirty, isValid }) => {
                                ...
                                return (
                                    <div className='col-12'>
                                        {loader == true ? <Loading /> :
                                            <Form className='row'>
                                                {event && event.memberType == "team" ? <div className="form-floating mb-4">
                                                ...
                                                <div className='col-lg-6 col-sm-6 col-xs-12 col-xl-6 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        <ErrorMessage name="fullName" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                    </div>
                                                </div>
                                                <div className='col-lg-6 col-sm-6 col-xs-12 col-xl-6 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        />
                                                        <label>Email</label>
                                                        <ErrorMessage name="email" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>

                                                    </div>
                                                </div>
                                                <div className='col-lg-6 col-sm-6 col-xs-12 col-xl-6 p-1'><div className="form-floating mb-1">
                                                    <Field
                                                        type="number"
                                                        name="mobile"
                                                        className="form-control"
                                                    />
                                                    <label>Mobile</label>
                                                    <ErrorMessage name="mobile" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                ...
                                                <div className='col-lg-6 col-sm-6 col-xs-12 col-xl-6 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        <label>Gender</label>
                                                        <ErrorMessage name="gender" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                ...
                                                <div className='col-lg-12 col-sm-12 col-xs-12 col-xl-12 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        ></Field>
                                                        <label>Address</label>
                                                        <ErrorMessage name="address" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                ...
                                                <div className='col-lg-4 col-sm-4 col-xs-12 col-xl-4 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        <label>State</label>
                                                        <ErrorMessage name="state" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                    </div>
                                                </div>
                                                <div className='col-lg-4 col-sm-4 col-xs-12 col-xl-4 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        >
                                                            {
                                                                s_a[state_arr.indexOf(values && values.state)] && s_a[state_arr.indexOf(values && values.state)].split("|") && s_a[state_arr.indexOf(values && values.state) + 1].split("|").map((item, index) => {
                                                                    return (
                                                                        <option>{item}</option>
                                                                    )
                                                                })
                                                        ...
                                                        <label>City</label>
                                                        <ErrorMessage name="city" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                    </div>
                                                </div>
                                                <div className='col-lg-4 col-sm-4 col-xs-12 col-xl-4 p-1'>
                                                    <div className="form-floating mb-1">
                                                        ...
                                                        <label>Pincode</label>
                                                        <ErrorMessage name="pincode" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                    </div>
                                                </div>
                                                ...
                                                {event && event.ticketSystem == true ?
                                                    <div className='col-lg-6 col-sm-6 col-xs-12 col-xl-6 p-1'><div className="form-floating mb-1">
                                                        <Field
                                                            ...
                                                            max={1000}
                                                            onChange={(e) => {
                                                                setFieldValue("amount", totalAmount(e.target.value));
                                                                setFieldValue("noOfTickets", e.target.value)
                                                            }}
                                                        />
                                                        <label>No. of Tickets</label>
                                                        <ErrorMessage name="noOfTickets" render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                    </div>
                                                    </div> : null}
                                                                        ...
                                                                        <div className='row row-xs align-items-center'>
                                                                            <div className='col-sm-4'>
                                                                                <div className='form-floating mt-2'>
                                                                                    ...
                                                                                    <ErrorMessage name={`members.${index}.name`} render={(msg) => <small className='text-danger'>{msg}</small>}></ErrorMessage>
                                                ...
                                                <div className="form-group mt-2">
                                                    <h3 className='text-primary text-end'>₹{values.amount}</h3>
                                                    ...
                                                    <Field
                                                        ...
                                                        id="flexCheckDefault"
                                                    ></Field>
                                                    <span htmlFor="flexCheckDefault">I understand that I am responsible for reading and understanding the terms and conditions of the event or service I am registering for.</span>
                                                </div>
