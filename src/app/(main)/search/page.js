...
function Search() {
    ...
    useEffect(() => {
        setLoader(true)
        axios.post(`${config.API_URL}/landing/search`, { referrer, keyword, state, activity }).then((result) => {
            setLoader(false)
                    ...
                    <div className="container">
                        <div className="row justify-content-center">
                            <div className="col-xl-12 col-lg-12">
                                <h5 className="mb-3" style={{textTransform : "capitalize"}}>Explore All {referrer}</h5>
                            </div>
                        </div>
                            ...
                            {data && data.map((item, index) => {
                                return (
                                    <div className="col-xl-4 col-lg-4 col-md-4 col-sm-6" key={index}>
                                        {referrer == "venue" ? <SimpleCard item={item} /> : <EventsCard item={item} />}
                                    </div>
                                )
                            })}
