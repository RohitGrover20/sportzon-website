...
function Transaction() {
	...
	useEffect(() => {
		axios
			.get(`${config.API_URL}/landing/payments/recent-transactions`, { withCredentials: true })
			.then((result) => {
								...
								<tr key={index}>
									...
									<td>
										{item.value && item.value.status == "captured" ? (
											<span className="label text-success bg-light-success">Completed</span>
										) : item.value && item.value.status == "failed" ? (
											<span className="label text-danger bg-light-danger">Failed</span>
										) : (
