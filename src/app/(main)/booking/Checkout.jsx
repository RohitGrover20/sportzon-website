...
function Checkout(props) {
  ...
  const [redeemedCoins, setRedeemedCoins] = useState(0); // State for redeemed coins
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(
          `${config.API_URL}/landing/wallet/${user?.data?._id}/total-coins`,
          {
  ...
  const updateWallet = async () => {
    try {
      await axios.post(
        `${config.API_URL}/landing/wallet/deduct-coins`,
  ...
  const payment = async () => {
    ...
    if (user && user.code == "unauthorised") {
      ...
      const _data = (subtotal + gst - redeemedCoins) * 100;
      const bookingData = {
        data: {
          bookingType: "arena",
    ...
    var options = {
      ...
      handler:  function (response) {
        ...
        axios
          ...
          .then((res) => {
            updateWallet(); // Deduct coins from wallet after successful payment
            toast.success(res.data.message, {
              position: "top-right",
              autoClose: 2000,
              onClose: () => {
        ...
        {cart?.length == 0 ? (
          ...
          <>
            ...
            <ul className="list-unstyled py-3 mb-0 border-top">
              ...
              <li className="d-flex justify-content-between mb-2 text-dark fw-bold">
                Discount (Redeem Coins):
                <span className="fw-semibold ms-2"> - Rs. {redeemedCoins}</span>
              </li>
              <li className="d-flex justify-content-between mb-2 text-dark fw-bold">
                Total:
                <span className="fw-semibold ms-2">
                  Rs. {subtotal + gst - redeemedCoins}
                </span>
              </li>
              <li className="d-flex justify-content-between mb-0 text-dark fw-bold">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (value > totalCoins?.totalCoins) {
                      setRedeemedCoins(0); // Reset to 0 or handle as needed
                      setCoinError(
                        "Redeemed coins cannot exceed total available coins."
                      );
                    } else if (value > totalAmount - minAmount) {
                      setRedeemedCoins(totalAmount - minAmount); // Set to max allowable coins
                      setCoinError(
                        "Redeemed coins cannot reduce the payment amount to less than 1 rupee."
                      );
                    } else {
                      setRedeemedCoins(value);
                      setCoinError(""); // Clear error if value is valid
                    }
                  }}
                  min="0"
                  max={totalCoins?.totalCoins}
                  className="form-control"
                />
              </li>
              {coinError && <p className="text-danger text-end">{coinError}</p>}{" "}
              {/* Display error message */}
              <p className="text-danger text-end">
                You have total credit coins : {totalCoins?.totalCoins}
              </p>
            </ul>
            <div className="border-top pt-4">
              ...
              <button
                ...
                disabled={subtotal + gst - redeemedCoins < 0}
              >
                Proceed INR {subtotal + gst - redeemedCoins}
              </button>
            </div>
          </>
        )}
