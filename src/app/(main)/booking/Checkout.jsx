...
function Checkout(props) {
		...
		if (user && user.code == "unauthorised") {
			...
			axios
				...
				.then((res) => {
					handleOpenRazorpay({
						...res.data.data,
						values: { ...user.data, bookingType: "arena", arena: venue && venue._id, court: cart },
					})
				})
				.catch((err) => {
					setLoading(false)
	...
	return (
		<div className="col-xl-4 col-lg-4 col-lg-offset-1">
			<div className="exploi  rounded py-3 px-3 border" style={{ backdropFilter: "blur(10px)" }}>
				{cart.length == 0 ? (
					<>
						<h3 className="offcanvas-title d-flex align-items-center mb-1">Your Cart is empty</h3>
						<i className="fa fa-shopping-cart text-orange" style={{ fontSize: "40px" }} />
					</>
				) : (
					<>
						<div className="d-flex w-100 justify-content-between align-items-center border-bottom pb-2 pb-sm-2 mb-3">
							<h2 className="offcanvas-title d-flex align-items-center mb-1">Your Cart</h2>
						</div>
						{/* Single Product */}
						{cart &&
										...
										<div className="w-100 pt-1">
											<div className="d-flex mb-3">
												...
												<div className="text-end ms-auto">
													<div className="fs-10 font--bold mb-2">{item && item.court}</div>
													<div
														className="fs-4 p-0"
